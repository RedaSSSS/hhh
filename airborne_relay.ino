/**
 * ============================================================
 *  AIRBORNE RELAY — ESP32-S3 (Minimal Stable Version)
 *  Search & Rescue Proof of Concept
 * ============================================================
 *  This version prioritizes STABILITY on ESP32-S3:
 *    - Connects to Wi-Fi (STA mode only first)
 *    - Starts AP after STA is connected
 *    - Runs probe sniffer with safe delays
 *    - Reports via UDP to command server
 * ============================================================
...
void snifferCallback(void* buf, wifi_promiscuous_pkt_type_t type) {
  ...
  // Check frame type = Management (0), subtype = Probe Request (4)
...
void setup() {
  Serial.begin(115200);
  delay(2000);  // ESP32-S3 USB-CDC needs time
  Serial.println();
  Serial.println("=== AIRBORNE RELAY BOOT (S3) ===");

  // Step 1: Connect to Wi-Fi as station
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("\n[STA] WARNING: Not connected. Will retry later.");
  }

  // Step 2: Now switch to AP+STA and start AP
  delay(500);
  WiFi.mode(WIFI_AP_STA);
  delay(500);
  
  // Reconnect STA after mode switch
  if (WiFi.status() != WL_CONNECTED) {
    if (strlen(STA_PASSWORD) > 0) {
      WiFi.begin(STA_SSID, STA_PASSWORD);
    } else {
  ...
  // Step 4: Start sniffer AFTER everything else is stable
  delay(1000);
...
void loop() {
  ...
  if (now - lastReportMs >= REPORT_INTERVAL_MS) {
    ...
    // Temporarily disable sniffer while we send
    if (snifferActive) {
  ...
  delay(100);  // Generous yield to prevent watchdog
}
